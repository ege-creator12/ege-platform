...
function bankPatterns(subject,line){
  if(subject==='chemistry')return [`chemistry-bank-${CHEMISTRY_BANK_VERSION}-line${line}-%`];
  if(Number(line)===26)return [`biology-bank-v${BIOLOGY_BANK_VERSION}-line26-hard-%`];
  return [`biology-bank-v${BIOLOGY_BANK_VERSION}-line${line}-%`];
}
...
async function assertChemistryLineIds(ids,line,subject){
 if(!ids.length)return;
 const marks=ids.map(()=>'?').join(',');
 const selected=await rows(`SELECT id,external_key,exam_line,subject_id FROM questions WHERE id IN (${marks})`,...ids);
 const prefix=`chemistry-bank-${CHEMISTRY_BANK_VERSION}-line${line}-`;
 const bad=selected.filter(q=>Number(q.exam_line)!==Number(line)||Number(q.subject_id)!==Number(subject)||!String(q.external_key||'').startsWith(prefix));
 if(selected.length!==ids.length||bad.length){
  console.error('strict-chemistry-line-violation',{requestedLine:line,ids,bad});
  throw Object.assign(new Error(`Защита линии ${line}: обнаружено задание химии из устаревшего или другого банка`),{status:500});
 }
}
