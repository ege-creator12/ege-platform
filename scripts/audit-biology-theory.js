...
const ZOOLOGY_REQUIRED_TERMS=['ложноножки','конъюгация','нет настоящих органов','радиально','паренхим','финн','прудовик','промежуточного хозяина нет','первичная полость','замкнутая кровеносная','мантия','головоногих кровеносная система замкнутая','линя','две пары усиков','четыре пары ходильных ног','гемолимфа','трахей','куколк','хорда','настоящего сердца нет','один круг','боковая линия','два круга','частичное смешение','крокодилов сердце четырёхкамерное','амнион','газообмен идёт в лёгких','четырёхкамерное сердце','молочные железы','яйцекладущие','диафрагма','эктотерм','факт','механизм','следствие'];
...
function auditBiologyTheory(course,publicRoot=resolve(ROOT,'public')){
 ...
 const zooTopics=allTopics.filter(t=>ZOOLOGY_TOPICS.includes(t.slug)), zooLessons=zooTopics.flatMap(t=>lessons(t)), zooQuestions=zooTopics.flatMap(t=>t.questions||[]), zooCorpus=JSON.stringify(zooLessons).toLocaleLowerCase('ru');
 ...
 for(const term of ZOOLOGY_REQUIRED_TERMS)if(!zooCorpus.includes(term))throw new Error(`zoology mandatory concept missing: ${term}`);
 for(const lesson of zooLessons){const types=new Set(lesson.blocks.map(b=>b.type));for(const type of ['heading','text','algorithm','exam_trap','ege_example','summary'])if(!types.has(type))throw new Error(`${lesson.slug}: zoology block missing: ${type}`);if(lesson.blocks.filter(b=>b.type==='text').length<3)throw new Error(`${lesson.slug}: zoology explanation is not sufficiently connected`);if(!JSON.stringify(lesson).includes('EGE_REQUIRED'))throw new Error(`${lesson.slug}: EGE_REQUIRED marker missing`);const entry=auditText.split(/(?=^## )/m).find(x=>x.includes(`lesson: ${lesson.slug}`));if(!entry)throw new Error(`${lesson.slug}: zoology audit entry missing`);for(const field of [...AUDIT_FIELDS,'requiredPracticeMechanics'])if(!entry.includes(`- ${field}:`))throw new Error(`${lesson.slug}: zoology audit field missing: ${field}`);if(!entry.includes('- statusAfter: FULL'))throw new Error(`${lesson.slug}: zoology FULL was not manually recorded`)}
