(()=>{
  'use strict';
  if(window.__OSNOVA_HOMEWORK_BATCH_AI__)return;
  window.__OSNOVA_HOMEWORK_BATCH_AI__=true;
  ...
  let finalizing=new Map();

  const style=document.createElement('style');
  style.textContent=`
    .homework-batch-note{margin:14px 0;padding:13px 14px;border:1px solid rgba(91,232,151,.18);border-radius:13px;background:rgba(75,210,132,.07);font-size:12px;line-height:1.5;color:var(--muted)}
  ...
  function plural(n,a,b,c){n=Math.abs(Number(n)||0)%100;const x=n%10;return n>10&&n<20?c:x>1&&x<5?b:x===1?a:c}
  ...
  function renderSaved(sessionId,result){
    const answered=numberValue(result.session,'answeredCount','answered_count');
    const total=numberValue(result.session,'targetQuestions','target_questions');
    app.innerHTML=shell(`<div class="training"><div class="card question-card homework-saved-card"><div class="eyebrow">Домашняя работа</div><h1>Ответ сохранён</h1><p class="subtitle">Пока он <b>не проверяется</b> и не считается ни верным, ни ошибкой. ОСНОВА AI проверит все ответы одним запросом только после последнего задания.</p><div class="homework-save-progress"><b>${answered}</b><span>из ${total||'—'} заданий заполнено</span></div><div class="actions"><button class="btn" id="homework-batch-next">Следующее задание →</button><button class="btn ghost" id="homework-batch-exit">Выйти и продолжить позже</button></div></div></div>`);
    bindShell();
    document.querySelector('#homework-batch-next').onclick=()=>trainingQuestion(sessionId);
    document.querySelector('#homework-batch-exit').onclick=()=>go('homework');
  }

  function checkingScreen(message='Проверяем всю домашнюю работу…'){
    app.innerHTML=shell(`<div class="training"><div class="card question-card homework-batch-checking"><div class="loader-dot"></div><div class="eyebrow">ОСНОВА AI · единая проверка</div><h1>${esc(message)}</h1><p>Все ответы уже сохранены. Сейчас они отправляются на проверку <b>одним пакетным AI-запросом</b>. После этого появятся баллы и общий разбор ошибок.</p></div></div>`);
    bindShell();
  }
  ...
  async function finalizeHomework(sessionId){
    ...
    const work=(async()=>{
      ...
      try{
        ...
        metaCache.delete(id);
        return trainingSummary(result.session||{id});
      }catch(error){
        app.innerHTML=shell(`<div class="training"><div class="card question-card"><div class="eyebrow">Домашняя работа сохранена</div><h1>Проверка не завершилась</h1><p class="subtitle">${esc(error.message)}</p><div class="homework-batch-note"><b>Ответы не потеряны.</b> Они сохранены без оценки и не считаются ошибками, пока общая AI-проверка не завершится.</div><div class="actions"><button class="btn" id="homework-batch-retry">Повторить проверку</button><button class="btn ghost" id="homework-batch-later">Вернуться к домашним</button></div></div></div>`);
        bindShell();
  ...
  async function renderHomeworkQuestion(sessionId,draft=[]){
    ...
    try{
      const d=await request(`/training/sessions/${Number(sessionId)}/next`);
      if(d.done)return finalizeHomework(sessionId);
      ...
      app.innerHTML=shell(`<div class="training"><div class="train-top"><button class="back" data-exit>← К домашним заданиям</button><span class="pill">${answered+1} из ${total}</span></div>${typeof pct==='function'?pct(progress):''}<div class="card question-card"><div class="q-meta"><span class="pill">${esc(typeLabel)}</span><span class="pill">${esc(q.topic||'Биология')}</span></div><div class="homework-secure-badge"><b>Домашняя работа</b><span>Сейчас ответ только сохраняется. Проверка и разбор появятся после выполнения всей работы.</span></div>${q.instruction?`<p class="question-instruction">${esc(q.instruction)}</p>`:''}<h2>${esc(q.prompt||'')}</h2>${controls}<div class="homework-batch-note"><b>Без мгновенной проверки:</b> правильный ответ, балл и ошибки до конца работы не показываются.</div><div class="actions training-actions"><button class="btn" id="homework-batch-submit">Сохранить ответ →</button></div></div></div>`);
      bindShell();
      ...
      document.querySelector('#homework-batch-submit').onclick=async()=>{
        ...
        const answer=QuestionControls.read(document.querySelector('.question-card'),q);
        if(!answer.length||answer.some(x=>!String(x).trim()||x==='invalid'))return notify('Заполните ответ полностью');
        button.disabled=true;button.textContent='Сохраняем…';
        try{
          const result=await request(`/teacher/homework/sessions/${Number(sessionId)}/save`,{method:'POST',body:JSON.stringify({questionId:q.id,answer,duration:Math.round((Date.now()-started)/1000)})});
          if(result.done)return finalizeHomework(sessionId);
          return renderSaved(sessionId,result);
        }catch(error){
          if(error.status===409&&/сохранён/i.test(error.message))return trainingQuestion(sessionId);
          notify(error.message);button.disabled=false;button.textContent='Сохранить ответ →';
        }
      };
    }catch(error){
  ...
  if(originalTrainingSummary){
    trainingSummary=async function(session){
      ...
      if(!sessionId)return originalTrainingSummary(session);
      let data;
      try{data=await request(`/teacher/homework/sessions/${sessionId}/summary`)}catch(error){if(error.status===404)return originalTrainingSummary(session);throw error}
      if(!data?.batchAi)return originalTrainingSummary(session);
  ...
  function patchTeacherCopy(){
    document.querySelectorAll('.homework-check-note').forEach(note=>{
      note.innerHTML='<b>Проверяет ОСНОВА AI:</b> ответы ученика сначала только сохраняются. После последнего задания вся домашняя работа проверяется одним пакетным AI-запросом, затем появляются баллы и разбор ошибок.';
    });
  }
  ...
  addEventListener('hashchange',schedule);setTimeout(schedule,400);
})();
