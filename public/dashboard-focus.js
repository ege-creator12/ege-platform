(()=>{
  ...
  function missionHtml(plan){
    ...
    const score=plan.scoreEstimate==null?'—':plan.scoreEstimate;
    const gap=plan.scoreEstimate==null?'После диагностики покажем разницу до цели':Math.max(0,Number(plan.targetScore)-Number(plan.scoreEstimate));
    if(!mission||mission.rest){
      return `<section id="${ROOT_ID}" class="daily-mission rest"><div class="daily-mission-main"><div class="daily-mission-top"><span class="eyebrow">Сегодня · ${esc(subjectLabel(plan.subjectSlug))}</span><span class="daily-goal">Цель ${esc(plan.targetScore)}+</span></div><h2>Лёгкий день: восстановление и повторение</h2><p>${esc(plan.coachNote||'Можно отдохнуть или коротко повторить ошибки без новой тяжёлой темы.')}</p><div class="daily-score"><span>Сейчас <b>${esc(score)}</b></span><span>До цели <b>${typeof gap==='number'?gap:'—'}</b></span></div></div><div class="daily-mission-actions"><button class="btn" data-daily-map>Открыть карту ЕГЭ</button><button class="btn ghost" data-daily-plan>Открыть план</button></div></section>`;
    }
    return `<section id="${ROOT_ID}" class="daily-mission"><div class="daily-mission-main"><div class="daily-mission-top"><span class="eyebrow">Сегодня · ${esc(subjectLabel(plan.subjectSlug))}</span><span class="daily-goal">Цель ${esc(plan.targetScore)}+</span></div><h2>${esc(mission.title||`Линия ${mission.line}`)}</h2><p>Один готовый блок на сегодня: без выбора темы и без лишнего планирования.</p><div class="daily-breakdown"><span><b>${esc(mission.theoryMinutes||0)}</b> мин теория</span><span><b>${esc(mission.practiceMinutes||0)}</b> мин практика</span><span><b>${esc(mission.reviewMinutes||0)}</b> мин повторение</span><span><b>${esc(mission.questions||0)}</b> заданий</span></div><div class="daily-score"><span>Сейчас <b>${esc(score)}</b></span><span>До цели <b>${typeof gap==='number'?gap:'—'}</b></span><span>Нагрузка <b>${esc(plan.minutesPerDay)} мин</b></span></div></div><div class="daily-mission-actions"><button class="btn daily-start" data-daily-start data-subject="${esc(plan.subjectSlug)}" data-line="${esc(mission.line)}" data-count="${esc(mission.questions||8)}">Начать занятие →</button><button class="btn ghost" data-daily-map>Карта линий</button><button class="btn ghost" data-daily-plan>Весь план</button></div></section>`;
  }
  ...
  async function startLine(button){
    ...
    try{
      ...
      const body=subject==='chemistry'?{examLine:line,mode:'adaptive',targetQuestions:count}:{examLine:line,topicId:0,mode:'adaptive',targetQuestions:count};
