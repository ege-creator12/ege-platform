...
async function api(req,res,path){
 try{
  ...
  if(path.match(/^\/api\/training\/sessions\/\d+\/answer$/)&&req.method==='POST'){
   const u=await auth(req,res);if(!u)return;const id=numericId(path.split('/')[4]),b=await body(req),questionId=numericId(b.questionId);if(!id||!questionId)return json(res,400,{error:'Некорректный идентификатор'});const item=await row(`SELECT tsq.position,tsq.state,q.* FROM training_session_questions tsq JOIN training_sessions s ON s.id=tsq.session_id JOIN questions q ON q.id=tsq.question_id WHERE s.id=? AND s.user_id=? AND s.status='active' AND q.id=?`,id,u.id,questionId);
