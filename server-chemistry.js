...
async function createChemistryTraining(userId,b){
 ...
 let ids=(await rows(`SELECT q.id FROM questions q ${latest} WHERE q.subject_id=? AND q.exam_line=? AND q.active=1 AND q.published=1 AND ${filter} ORDER BY CASE WHEN a.id IS NOT NULL AND a.correct=FALSE THEN 0 WHEN a.id IS NULL THEN 1 ELSE 2 END,q.difficulty,RANDOM() LIMIT ?`,userId,subject.id,line,target)).map(x=>Number(x.id));
 if(!ids.length&&mode!=='adaptive')ids=(await rows(`SELECT q.id FROM questions q WHERE q.subject_id=? AND q.exam_line=? AND q.active=1 AND q.published=1 ORDER BY q.difficulty,RANDOM() LIMIT ?`,subject.id,line,target)).map(x=>Number(x.id));
