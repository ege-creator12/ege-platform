...
const expectedType={
  1:'text',2:'multiple',3:'text',4:'text',5:'text',6:'matching',7:'multiple',8:'matching',9:'text',10:'matching',
  11:'multiple',12:'sequence',13:'text',14:'matching',15:'multiple',16:'sequence',17:'multiple',18:'multiple',19:'matching',
  20:'text',21:'multiple',22:'text',23:'text',24:'text',25:'text',26:'text',27:'text',28:'text'
};
...
for(let line=1;line<=28;line++){
  test(`biology strict v6 line ${line} has correct mechanics for 24 variants`,()=>{
    ...
    if([2,7,11,15,17,18,21].includes(line))variants.forEach((item,n)=>assert.ok(item.options?.length>=5,`line ${line} variant ${n+1}: choice options missing`));
    if([6,8,10,14,19].includes(line))variants.forEach((item,n)=>assert.ok(item.content?.left?.length>=4&&item.content?.right?.length>=2,`line ${line} variant ${n+1}: matching data missing`));
