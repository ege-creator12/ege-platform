...
function createChemistryMockExamService(db,registry){
 ...
 async function handle(req,res,path,user,json,body){
  ...
  try{
   ...
   if(parts[1]==='help'&&req.method==='POST'){if(attempt.status!=='in_progress')return send(409,{error:'Помощь доступна во время прохождения'});const b=await body(req);return send(200,await helpFor(attempt,Number(b.itemId),b.action||'hint'));}
