  ...
  for (const [si,section] of course.sections.entries()) {
    ...
    for (const [ti,topic] of (section.topics||[]).entries()) {
      ...
      for (const [lessonPosition,l] of topicLessons.entries()) {
        ...
        for (const [position,block] of (l.blocks||[]).entries()) { await db.run(`INSERT INTO lesson_blocks(lesson_id,type,content_json,position) VALUES(?,?,?,?)`,lessonId,block.type,JSON.stringify(block.content),position); if(spec) await db.run(`INSERT INTO content_coverage(spec_item_id,entity_type,entity_id,coverage_kind) VALUES(?,'block',?,'theory') ON CONFLICT(spec_item_id,entity_type,entity_id,coverage_kind) DO NOTHING`,spec.id,(await db.row('SELECT id FROM lesson_blocks WHERE lesson_id=? AND position=?',lessonId,position)).id); }
      }
      ...
      for (const q of topic.questions||[]) {
        ...
        const status=q.contentStatus||'draft', answer=JSON.stringify(q.answer), explanation=JSON.stringify({short:q.explanation,fullSolution:q.explanation,commonMistake:q.commonMistake||'',theoryReference:lessonSlug});
