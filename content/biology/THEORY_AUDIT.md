...
> Исторический отчёт. Метки FULL/approved ниже не доказывают полноту курса: повторная содержательная проверка обнаружила пропуски. Актуальные подтверждённые изменения и незакрытый объём перечислены в [COMPLETENESS_REVIEW.md](COMPLETENESS_REVIEW.md). Автотесты проверяют целостность, но не заменяют предметную экспертизу.
