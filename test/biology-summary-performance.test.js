...
function fixture() {
  ...
  const questions=[
    [1,1,1,1,'biology-bank-v7-line1-valid',1,1,'multiple','Первый',1],
    [2,1,1,1,'biology-bank-v7-line1-fixed',1,1,'multiple','Второй',1],
    [3,1,1,2,'biology-bank-v7-line2-valid',1,1,'multiple','Третий',1],
    [4,2,2,1,'biology-bank-v7-line1-other-subject',1,1,'multiple','Химия',1],
    [5,1,1,1,'biology-bank-v6-line1-old',1,1,'multiple','Старый',1],
    [6,1,1,1,'biology-bank-v7-line10-mismatch',1,1,'multiple','Другая линия',1],
    [7,1,1,1,'biology-bank-v7-line1-inactive',0,1,'multiple','Неактивный',1],
    [8,1,1,2,'biology-bank-v7-line2-hidden',1,0,'multiple','Скрытый',1],
    [9,1,1,28,'biology-bank-v7-line28-valid',1,1,'multiple','Последний',1],
  ];
...
test('28-line summary uses four queries while preserving detail progress and strict bank isolation',async()=>{
  ...
  try {
    ...
    assert.deepEqual(result.lines[0].progress.wrongQuestionRefs,['biology-bank-v7-line1-inactive','biology-bank-v7-line1-valid']);
    ...
    assert.deepEqual(other.lines[1].progress.wrongQuestionRefs,['biology-bank-v7-line2-valid']);
    assert.equal(result.strictBank,true);
    assert.equal(result.bankVersion,7);
