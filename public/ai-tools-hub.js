(()=>{
  ...
  function card(icon,title,text,status,provider,buttons,wide=false){
    return `<article class="card aihub-card ${wide?'aihub-expert':''}"><div class="aihub-card-top"><span class="aihub-icon">${icon}</span><span class="aihub-status ${status==='Скоро'?'soon':''}">${safe(status)}</span></div><h3>${safe(title)}</h3><p>${safe(text)}</p><footer><span class="aihub-provider">${safe(provider)}</span>${buttons}</footer></article>`;
  }
  ...
  function renderHub(){
    const html=`<div class="aihub"><section class="card aihub-hero"><span class="eyebrow">ОСНОВА · AI</span><h1>AI-инструменты</h1><p>Нейросети собраны отдельно от обычной подготовки. Теория, задания, пробники, статистика и прогресс работают без AI; здесь находятся только функции, которые обращаются к нейросетям.</p></section>
      ...
      <div class="aihub-grid">
        ${card('✦','AI PRO','Персональный репетитор и куратор: объясняет темы, видит статистику, слабые линии и помогает выбрать следующий шаг.','Работает','Gemini','<button class="btn" data-ai-route="pro">Открыть AI PRO</button>')}
        ${card('✓','Эксперт второй части','Проверяет развёрнутые ответы после пробника по критериям конкретного задания и поэлементной методике ФИПИ. Балл автоматически попадает в результат.','Работает','Cerebras · gpt-oss-120b','<button class="btn" data-ai-route="mocks">Биология</button><button class="btn ghost" data-ai-route="chemistry/mocks">Химия</button>',true)}
        ${card('⌁','Разбор ошибок AI','Находит повторяющиеся ошибки по статистике ученика и объясняет, что именно сейчас выгоднее исправить.','Работает','Gemini · AI PRO','<button class="btn ghost" data-ai-route="pro">Разобрать ошибки</button>')}
        ${card('▧','Фото задания','Можно отправить фотографию задания в AI PRO и разобрать её вместе с репетитором.','Работает','Gemini · AI PRO','<button class="btn ghost" data-ai-route="pro">Открыть</button>')}
        ${card('↗','AI-планировщик','Строит и перестраивает план подготовки по цели, сроку до ЕГЭ, ошибкам и фактическому темпу.','Работает','Gemini · AI PRO','<button class="btn ghost" data-ai-route="pro">Мой план</button>')}
        ${card('◎','Глубокий AI-разбор пробника','Соберёт ошибки всего варианта в один вывод: почему потеряны баллы и какие 2–3 действия дадут лучший прирост.','Скоро','AI','<button class="btn ghost" disabled>В разработке</button>')}
      </div>
