...
async function progressHistory(userId,subjectSlug){
 ...
 const attempts=await rows(`SELECT a.correct,a.duration_seconds,a.created_at,q.exam_line
   FROM attempts a JOIN questions q ON q.id=a.question_id
   WHERE a.user_id=? AND q.subject_id=? ORDER BY a.id DESC LIMIT 1200`,userId,sid);
 const nowStart=weekStart(new Date());
 const weeks=[];
 for(let i=7;i>=0;i--){
  const start=new Date(nowStart);start.setDate(start.getDate()-i*7);
  const end=new Date(start);end.setDate(end.getDate()+7);
  weeks.push({start,date:dateKey(start),label:weekLabel(start),end,solved:0,correct:0,seconds:0,accuracy:0});
 }
 for(const attempt of attempts){
  const when=new Date(attempt.created_at);if(!Number.isFinite(when.getTime()))continue;
  const bucket=weeks.find(w=>when>=w.start&&when<w.end);if(!bucket)continue;
  bucket.solved++;bucket.correct+=(attempt.correct===true||attempt.correct===1||attempt.correct==='1'||attempt.correct==='true')?1:0;
 ...
 const history=weeks.map((week,index)=>{
  totalSolved+=week.solved;totalCorrect+=week.correct;
  ...
  const activityScore=Math.min(15,Math.round(week.solved/2));
  const estimatedScore=Math.max(0,Math.min(100,Math.round(rollingAccuracy*0.88+activityScore)));
  return {week:index+1,date:week.date,label:week.label,solved:week.solved,accuracy,minutes:Math.round(week.seconds/60),estimatedScore};
 });
 const current=history.at(-1)||{solved:0,accuracy:0,minutes:0,estimatedScore:0};
 const previous=history.at(-2)||{solved:0,accuracy:0,minutes:0,estimatedScore:0};
 return {subjectSlug,history,current,delta:{solved:current.solved-previous.solved,accuracy:current.accuracy-previous.accuracy,score:current.estimatedScore-previous.estimatedScore}};
}
...
async function reviewStatus(userId,subjectSlug){
 ...
 const wrong=list.filter(x=>!(x.correct===true||x.correct===1||x.correct==='1'||x.correct==='true')).length;
...
async function handleApi(req,res,url){
 ...
 if(path==='/api/ai-pro/progress'&&req.method==='GET'){
  const subjectSlug=validSubject(String(url.searchParams.get('subject')||'biology'));
  json(res,200,await progressHistory(user.id,subjectSlug));return true;
 }
 if(path==='/api/ai-pro/review-status'&&req.method==='GET'){
  const subjectSlug=validSubject(String(url.searchParams.get('subject')||'biology'));
  json(res,200,await reviewStatus(user.id,subjectSlug));return true;
 }
 if(path==='/api/ai-pro/smart-review'&&req.method==='POST'){
  const body=await readJson(req),subjectSlug=validSubject(String(body.subjectSlug||'biology'));
  ...
  const ids=[...new Set(candidates.map(x=>Number(x.id)).filter(Boolean))].slice(0,wanted);
...
async function start(){
 ...
 const server=http.createServer(async(req,res)=>{
  ...
  try{if(await handleApi(req,res,url))return;proxy(req,res);}catch(error){console.error('ai-pro-api',error);if(!res.headersSent)json(res,error?.status||500,{error:error?.status?error.message:'Не удалось выполнить AI PRO запрос'});}
 });
