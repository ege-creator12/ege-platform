...
async function questionPool(userId,topicId,mode,limit){
 ...
 const filter=mode==='new'?'a.id IS NULL':mode==='review'?"a.id IS NOT NULL AND a.next_review_at<=CURRENT_TIMESTAMP":mode==='mistakes'?'a.id IS NOT NULL AND a.correct=0':'1=1';
...
async function recordAnswer(userId,q,b){
 ...
 await run(`INSERT INTO lesson_progress(user_id,lesson_id,status,questions_solved,correct_answers,last_activity_at)
...
async function api(req,res,path){
 try{
  ...
  if(path==='/api/subjects'){const u=await auth(req,res);if(u)return json(res,200,{subjects:await rows('SELECT * FROM subjects ORDER BY id')})}
  ...
  if(path.match(/^\/api\/topics\/\d+$/)){const u=await auth(req,res);if(!u)return;const id=Number(path.split('/').pop()),topic=await row(`SELECT t.*,s.title subject,COALESCE(tp.mastery,0) mastery FROM topics t JOIN subjects s ON s.id=t.subject_id LEFT JOIN topic_progress tp ON tp.topic_id=t.id AND tp.user_id=? WHERE t.id=? AND t.published=1`,u.id,id);if(!topic)return json(res,404,{error:'Тема не найдена'});const stat=await row(`WITH RECURSIVE tree(id) AS (SELECT ? UNION ALL SELECT t.id FROM topics t JOIN tree ON t.parent_id=tree.id) SELECT COUNT(*) attempted,COALESCE(SUM(correct),0) correct FROM attempts a JOIN questions q ON q.id=a.question_id WHERE a.user_id=? AND q.topic_id IN (SELECT id FROM tree)`,id,u.id);return json(res,200,{topic,children:await rows('SELECT * FROM topics WHERE parent_id=? AND published=1 ORDER BY position',id),stat})}
  ...
  if(path==='/api/lesson-progress'){const u=await auth(req,res);if(!u)return;return json(res,200,{lessons:await rows('SELECT lp.*,t.title,t.slug FROM lesson_progress lp JOIN topics t ON t.id=lp.lesson_id WHERE lp.user_id=? ORDER BY lp.last_activity_at DESC',u.id)})}
  if(path.match(/^\/api\/lesson-progress\/\d+$/)&&req.method==='POST'){const u=await auth(req,res);if(!u)return;const lessonId=Number(path.split('/').pop()),lesson=await row("SELECT id FROM topics WHERE id=? AND kind='lesson' AND published=1",lessonId);if(!lesson)return json(res,404,{error:'Урок не найден'});const b=await body(req),theoryRead=Boolean(b.theoryRead),status=b.status==='completed'?'completed':'in_progress';await run(`INSERT INTO lesson_progress(user_id,lesson_id,status,theory_read,last_activity_at,completed_at) VALUES(?,?,?,?,CURRENT_TIMESTAMP,CASE WHEN ?='completed' THEN CURRENT_TIMESTAMP END)
    ON CONFLICT(user_id,lesson_id) DO UPDATE SET status=excluded.status,theory_read=CASE WHEN lesson_progress.theory_read OR excluded.theory_read THEN TRUE ELSE FALSE END,last_activity_at=CURRENT_TIMESTAMP,completed_at=CASE WHEN excluded.status='completed' THEN CURRENT_TIMESTAMP ELSE lesson_progress.completed_at END`,u.id,lessonId,status,theoryRead,status);return json(res,200,{progress:await row('SELECT * FROM lesson_progress WHERE user_id=? AND lesson_id=?',u.id,lessonId)})}
  if(path==='/api/training/sessions'&&req.method==='POST'){
   const u=await auth(req,res);if(!u)return;const b=await body(req),mode=trainingModes.has(b.mode)?b.mode:'adaptive',topicId=Math.max(0,Number(b.topicId)||0),target=Math.min(100,Math.max(1,Number(b.targetQuestions)||10));
   ...
   const created=await run('INSERT INTO training_sessions(user_id,topic_id,mode,target_questions) VALUES(?,?,?,?)',u.id,topicId||null,mode,target),sessionId=Number(created.lastInsertRowid);
  ...
  if(path.match(/^\/api\/training\/sessions\/\d+$/)&&req.method==='GET'){const u=await auth(req,res);if(!u)return;const session=await sessionPayload(Number(path.split('/').pop()),u.id);return session?json(res,200,{session}):json(res,404,{error:'Тренировка не найдена'})}
  if(path.match(/^\/api\/training\/sessions\/\d+\/next$/)&&req.method==='GET'){
   const u=await auth(req,res);if(!u)return;const id=Number(path.split('/')[4]),session=await row("SELECT * FROM training_sessions WHERE id=? AND user_id=? AND status='active'",id,u.id);if(!session)return json(res,404,{error:'Активная тренировка не найдена'});
   ...
   await run("UPDATE training_session_questions SET presented_at=COALESCE(presented_at,CURRENT_TIMESTAMP) WHERE session_id=? AND position=?",id,item.position);delete item.answer_json;
   return json(res,200,{session:{id,mode:session.mode,answeredCount:session.answered_count,targetQuestions:session.target_questions},question:item,options:await rows('SELECT value,label FROM question_options WHERE question_id=? ORDER BY position',item.id)})
  }
  if(path.match(/^\/api\/training\/sessions\/\d+\/answer$/)&&req.method==='POST'){
   const u=await auth(req,res);if(!u)return;const id=Number(path.split('/')[4]),b=await body(req),item=await row(`SELECT tsq.position,tsq.state,q.* FROM training_session_questions tsq JOIN training_sessions s ON s.id=tsq.session_id JOIN questions q ON q.id=tsq.question_id WHERE s.id=? AND s.user_id=? AND s.status='active' AND q.id=?`,id,u.id,Number(b.questionId));
   if(!item||item.state!=='pending')return json(res,409,{error:'Задание уже отвечено или не входит в тренировку'});const result=await recordAnswer(u.id,item,b);
   await run("UPDATE training_session_questions SET state='answered',attempt_id=?,answered_at=CURRENT_TIMESTAMP WHERE session_id=? AND position=?",result.attemptId,id,item.position);
   await run('UPDATE training_sessions SET answered_count=answered_count+1,correct_count=correct_count+? WHERE id=?',result.correct?1:0,id);
   const left=(await row("SELECT COUNT(*) n FROM training_session_questions WHERE session_id=? AND state='pending'",id)).n;if(!left)await run("UPDATE training_sessions SET status='completed',finished_at=CURRENT_TIMESTAMP WHERE id=?",id);
   return json(res,200,{...result,done:!left,session:await sessionPayload(id,u.id),stats:await stats(u.id)});
  }
  if(path.match(/^\/api\/training\/sessions\/\d+\/finish$/)&&req.method==='POST'){const u=await auth(req,res);if(!u)return;const id=Number(path.split('/')[4]);await run("UPDATE training_sessions SET status=CASE WHEN answered_count>=target_questions THEN 'completed' ELSE 'abandoned' END,finished_at=CURRENT_TIMESTAMP WHERE id=? AND user_id=? AND status='active'",id,u.id);const session=await sessionPayload(id,u.id);return session?json(res,200,{session}):json(res,404,{error:'Тренировка не найдена'})}
  if(path.match(/^\/api\/training\/result\/\d+$/)){const u=await auth(req,res);if(!u)return;const id=Number(path.split('/').pop()),result=await row('SELECT a.id,a.answer_json,a.correct,a.duration_seconds,a.created_at,a.next_review_at,a.interval_days,a.review_stage,a.result_json,q.prompt,q.explanation,q.answer_json expected_json,t.title topic FROM attempts a JOIN questions q ON q.id=a.question_id JOIN topics t ON t.id=q.topic_id WHERE a.id=? AND a.user_id=?',id,u.id);return result?json(res,200,{result}):json(res,404,{error:'Результат не найден'})}
  if(path==='/api/training/next'){const u=await auth(req,res);if(!u)return;const url=new URL(req.url,'http://x'),topic=Number(url.searchParams.get('topic'));let q=await row(`WITH RECURSIVE tree(id) AS (SELECT ? UNION ALL SELECT t.id FROM topics t JOIN tree ON t.parent_id=tree.id), last_attempt AS (SELECT a.*,ROW_NUMBER() OVER(PARTITION BY question_id ORDER BY id DESC) rn FROM attempts a WHERE user_id=?) SELECT q.* FROM questions q LEFT JOIN last_attempt a ON a.question_id=q.id AND a.rn=1 WHERE q.active=1 AND (?=0 OR q.topic_id IN (SELECT id FROM tree)) AND (a.id IS NULL OR a.next_review_at<=CURRENT_TIMESTAMP) ORDER BY CASE WHEN a.id IS NULL THEN 0 ELSE 1 END,a.next_review_at,q.difficulty,RANDOM() LIMIT 1`,topic,u.id,topic);if(!q)q=await row(`WITH RECURSIVE tree(id) AS (SELECT ? UNION ALL SELECT t.id FROM topics t JOIN tree ON t.parent_id=tree.id) SELECT q.* FROM questions q WHERE q.active=1 AND (?=0 OR q.topic_id IN (SELECT id FROM tree)) ORDER BY RANDOM() LIMIT 1`,topic,topic);if(!q)return json(res,404,{error:'Для этой темы пока нет заданий'});delete q.answer_json;return json(res,200,{question:q,options:await rows('SELECT value,label FROM question_options WHERE question_id=? ORDER BY position',q.id)})}
  if(path==='/api/training/answer'&&req.method==='POST'){
   const u=await auth(req,res);if(!u)return;const b=await body(req),q=await row('SELECT * FROM questions WHERE id=? AND active=1',Number(b.questionId));if(!q)return json(res,404,{error:'Задание не найдено'});
   return json(res,200,{...await recordAnswer(u.id,q,b),stats:await stats(u.id)});
  }
  ...
  if(path==='/api/admin/questions'&&req.method==='POST'){const u=await auth(req,res,true);if(!u)return;const b=await body(req);if(!b.prompt||!b.explanation||!Array.isArray(b.answer)||!b.topicId)return json(res,400,{error:'Заполните обязательные поля'});const x=await run('INSERT INTO questions(topic_id,type,prompt,explanation,difficulty,answer_json) VALUES(?,?,?,?,?,?)',b.topicId,b.type||'single',b.prompt,b.explanation,Math.min(3,Math.max(1,Number(b.difficulty)||1)),JSON.stringify(b.answer));for(const [i,label] of (b.options||[]).entries()) await run('INSERT INTO question_options(question_id,value,label,position) VALUES(?,?,?,?)',Number(x.lastInsertRowid),String(i),label,i);return json(res,201,{id:Number(x.lastInsertRowid)})}
  if(path.match(/^\/api\/admin\/questions\/\d+$/)&&req.method==='DELETE'){const u=await auth(req,res,true);if(u){await run('DELETE FROM questions WHERE id=?',Number(path.split('/').pop()));return json(res,200,{ok:true})}}
