...
function auditBiologyTheory(course,publicRoot=resolve(ROOT,'public')){
 ...
 if(humanTopics.length!==13||humanLessons.length!==63||humanQuestions.length!==315)throw new Error(`PHASE 3 coverage mismatch: ${humanTopics.length}/${humanLessons.length}/${humanQuestions.length}`);
 ...
 for(const lesson of humanLessons){const types=new Set(lesson.blocks.map(block=>block.type));for(const type of ['heading','text','comparison','algorithm','exam_trap','ege_example','experiment','summary'])if(!types.has(type))throw new Error(`${lesson.slug}: PHASE 3 block missing: ${type}`);if(lesson.blocks.filter(block=>block.type==='text').length<4)throw new Error(`${lesson.slug}: connected prose incomplete`);const entry=auditText.split(/(?=^## )/m).find(part=>part.includes(`lesson: ${lesson.slug}`));if(!entry)throw new Error(`${lesson.slug}: PHASE 3 audit entry missing`);for(const field of [...AUDIT_FIELDS,'requiredMechanisms','requiredCausalLinks','requiredPracticeMechanics',...ZOOLOGY_MANUAL_FIELDS])if(!entry.includes(`- ${field}:`))throw new Error(`${lesson.slug}: audit field missing: ${field}`);if(!entry.includes('- statusAfter: FULL')||!entry.includes('- manualContentReview: approved'))throw new Error(`${lesson.slug}: FULL/manual review record missing`)}
 const humanKeys=new Set(),humanPrompts=new Set(),humanLessonSlugs=new Set(humanLessons.map(lesson=>lesson.slug));
 for(const q of humanQuestions){if(humanKeys.has(q.key))throw new Error(`duplicate human question: ${q.key}`);humanKeys.add(q.key);const prompt=q.prompt.toLocaleLowerCase('ru').replace(/\s+/g,' ').trim();if(humanPrompts.has(prompt))throw new Error(`duplicate human prompt: ${q.key}`);humanPrompts.add(prompt);if(!humanLessonSlugs.has(q.lessonSlug))throw new Error(`${q.key}: broken lesson ref`);if(!q.explanation||!q.solutionSteps?.length)throw new Error(`${q.key}: missing explanation/steps`);if(q.options&&q.answer.some(answer=>/^\d+$/.test(answer)&&Number(answer)>=q.options.length))throw new Error(`${q.key}: invalid correct answer`);if(q.image&&!existsSync(resolve(publicRoot,q.image.replace(/^\//,''))))throw new Error(`${q.key}: broken question media`)}
 ...
 if(humanTypes.extended_answer<30||humanTypes.extended_answer>40||humanTypes.experiment<20||Object.values(humanDifficulties).some(value=>value<60))throw new Error('PHASE 3 difficulty/extended/experiment coverage incomplete');
 ...
 const svgFingerprints=new Set(); for(const asset of humanAssets){const svg=readFileSync(resolve(publicRoot,asset.path.replace(/^\//,'')),'utf8');if(!svg.includes('<title')||!svg.includes('<desc')||!svg.includes('viewBox=')||!svg.includes('preserveAspectRatio='))throw new Error(`${asset.key}: invalid accessible/responsive SVG`);if(asset.source!=='original'||!asset.alt||!asset.description)throw new Error(`${asset.key}: missing provenance/alt`);if((svg.match(/<text/g)||[]).length<5)throw new Error(`${asset.key}: diagram lacks readable biological labels`);const fp=svg;if(svgFingerprints.has(fp))throw new Error(`${asset.key}: duplicate decorative SVG`);svgFingerprints.add(fp)} for(const lesson of humanLessons.filter(lesson=>lesson.blocks.some(block=>block.type==='diagram'))){if(!lesson.blocks.some(block=>block.type==='text'&&block.content.text.startsWith('Как читать схему.')))throw new Error(`${lesson.slug}: diagram reading/EGE guidance missing`)}
