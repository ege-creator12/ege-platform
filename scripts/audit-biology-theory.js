...
function auditBiologyTheory(course,publicRoot=resolve(ROOT,'public')){
 ...
 for(const lesson of humanLessons){const types=new Set(lesson.blocks.map(block=>block.type));for(const type of ['heading','text','comparison','algorithm','exam_trap','ege_example','experiment','summary'])if(!types.has(type))throw new Error(`${lesson.slug}: PHASE 3 block missing: ${type}`);const entry=auditText.split(/(?=^## )/m).find(part=>part.includes(`lesson: ${lesson.slug}`));if(!entry)throw new Error(`${lesson.slug}: PHASE 3 audit entry missing`);for(const field of [...AUDIT_FIELDS,'requiredMechanisms','requiredCausalLinks','requiredPracticeMechanics',...ZOOLOGY_MANUAL_FIELDS])if(!entry.includes(`- ${field}:`))throw new Error(`${lesson.slug}: audit field missing: ${field}`)}
 ...
 if(evolutionTopics.length!==17||evolutionLessons.length!==17||evolutionQuestions.length!==110||evolutionAssets.length!==17)throw new Error(`PHASE 4 coverage mismatch: ${evolutionTopics.length}/${evolutionLessons.length}/${evolutionQuestions.length}/${evolutionAssets.length}`);
 ...
 if(ecologyTopics.length!==4||ecologyLessons.length!==20||ecologyQuestions.length!==100||ecologyAssets.length!==20)throw new Error(`PHASE 5 coverage mismatch: ${ecologyTopics.length}/${ecologyLessons.length}/${ecologyQuestions.length}/${ecologyAssets.length}`);
 ...
 for(const lesson of ecologyLessons){const types=new Set(lesson.blocks.map(block=>block.type));for(const type of ['heading','text','comparison','diagram','algorithm','exam_trap','ege_example','experiment','summary'])if(!types.has(type))throw new Error(`${lesson.slug}: PHASE 5 block missing: ${type}`);}
